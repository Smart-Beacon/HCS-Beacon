...
class facility extends Sequelize.Model {
    ...
    static init(sequelize) {
        return super.init({  // 첫번째 객체 인수는 테이블 필드에 대한 설정
                ...
                modelName: 'facility', /* 모델 이름을 설정. */
                tableName: 'facility', /* 데이터베이스의 테이블 이름. */
   ...
   // 다른 모델과의 관계
    static associate(db) { // 인자로 index.js에서 만든 여러 테이블이 저장되어있는 db객체를 받을 것이다.
        db.facility.hasMany(db.statement, {
            foreignKey: 'facId',
            sourceKey: 'facId',
            onDelete: 'cascade',
            onUpdate: 'cascade',
        });
        // db.facility (hasMany) db.statement = 1:N 관계 이다.  
        // db.facility는 가지고있다. 많이. db.statement를
    }
};
